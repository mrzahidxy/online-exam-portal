...
export const validateInteractiveTableTemplate = (
  ...
  const columnIds = template.columns.map((column) => column.id.trim());
  const rowIds = template.rows.map((row) => row.id.trim());
  const fillableIds = getInteractiveTableFillableCellIds(template);
  ...
  template.rows.forEach((row, rowIndex) => {
    ...
    row.cells.forEach((cell, cellIndex) => {
      if (
        (cell.type === "select" || cell.type === "drop_zone") &&
        cell.options.length === 0
      ) {
        errors.push(`${cell.type} cell at row ${rowIndex + 1}, column ${cellIndex + 1} needs at least one option.`);
      }
      if (cell.type === "drop_expression" && cell.options.length === 0) {
        errors.push(`drop_expression cell at row ${rowIndex + 1}, column ${cellIndex + 1} needs at least one option.`);
      }
    });
  });

  if (new Set(fillableIds).size !== fillableIds.length) {
    errors.push("Fillable cell IDs must be unique.");
  }

  const fillableIdSet = new Set(fillableIds);
  const modelAnswerKeys = Object.keys(template.modelAnswer ?? {});
  const unknownModelKeys = modelAnswerKeys.filter((key) => !fillableIdSet.has(key));
  if (unknownModelKeys.length > 0) {
    errors.push("Model answer keys must match fillable cell IDs.");
  }
